...
export function MilestoneInputForm() {
  ...
  async function submit() {
    ...
    if (!response.ok) {
      setState('idle');
      setMessage(payload.error ?? '확인한 날짜를 저장하지 못했어요.');
      return;
    }
    setState('saved');
    setMessage(`확인한 날짜를 저장했어요. 오늘 홈 기준: ${payload.careSurface?.surfaceCareDay ?? 'routine_day'}`);
    window.setTimeout(() => {
  ...
  return (
    <section className={`${styles.choiceSection} ${styles.interviewSlide}`} aria-labelledby="milestone-input-title">
      <h2 className={styles.sectionTitle} id="milestone-input-title">확인한 날짜만 먼저 남겨요</h2>
      <p className={styles.questionLead}>병원에서 다시 확인한 날짜가 생기면 나중에 새로 남길 수 있어요.</p>
      ...
      <label className="field-label" htmlFor="timeline-milestone">어떤 일정인가요?</label>
      ...
      <label className="field-label" htmlFor="timeline-notes">짧은 메모</label>
      ...
      <div className={styles.timelinePreview} aria-label="저장될 확인 날짜 미리보기">
        <strong>확인한 날짜</strong>
        <span>{startedAt} · {MILESTONES.find((item) => item.value === milestone)?.label} · {confirmedAt}</span>
      </div>

      <CtaButton disabled={state === 'saving'} onClick={submit} type="button">{state === 'saving' ? '저장 중' : '확인한 날짜 저장하고 홈 보기'}</CtaButton>
