...
export async function POST(request: NextRequest) {
  ...
  const rawCaptureText = buildPhotoCaptureText(imagePath, candidates);
  const capture = await store.createCapture(rawCaptureText);
  const offsets = mapSourceOffsets(rawCaptureText, candidates.map((candidate) => candidate.title));
  const rows = candidates.map((candidate, index): SplitCandidateInsertRow => ({
