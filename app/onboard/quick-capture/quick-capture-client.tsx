...
export function QuickCaptureClient() {
  ...
  async function submit() {
    ...
    if (!response.ok) {
      setError(payload.error ?? '빠른 기록을 저장하지 못했어요.');
      setSubmitting(false);
      return;
    }
    setComplete(payload);
  ...
  return (
    <section className={styles.choiceSection} aria-labelledby="quick-capture-title">
      ...
      {complete ? (
        <Notice tone="sage">저장됐어요. 홈에서 오늘 카드를 확인할 수 있어요. <a href={complete.redirectTo ?? '/home'}>홈으로 이동</a></Notice>
      ) : null}
      ...
      <CtaButton disabled={submitting} onClick={submit} type="button">오늘 카드 만들기</CtaButton>
    </section>
  );
}
