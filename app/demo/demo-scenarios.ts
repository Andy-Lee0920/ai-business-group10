...
export const DEMO_SCENARIOS: Record<IvfStage, DemoScenario> = {
  baseline_testing: {
    ...
    patient: {
      ...
      utilityCards: [
        card('baseline-checklist', 'checklist', '검사 결과 저장', '검사 결과 저장', { amh: '', afc: '', fsh: '', lh: '', e2: '' }),
  ...
  ovarian_stimulation: {
    ...
    patient: {
      ...
      utilityCards: [
        card('stim-medication', 'medication_card', '약 이름과 시간', '고날에프 225IU · 21:00', { name: '고날에프', dose: '225IU', time: '21:00' }, 'care'),
        card('stim-log', 'injection_log', '주사 기록', '예정 21:00 · 실제 기록 대기', { scheduled_time: '21:00', actual_time: '', administered_by: 'self', recorded_by: 'patient', confirmed_by_patient: false }, 'care'),
        card('stim-site', 'checklist', '주사 부위', '아랫배 좌우 교대'),
        card('stim-symptom', 'checklist', '증상 메모', '복부팽만/두통 기록', { discomfort: 2 }, 'emotional'),
      ],
  ...
  egg_retrieval: {
    ...
    patient: {
      ...
      utilityCards: [card('retrieval-procedure', 'checklist', '방문 체크', '금식·도착시간', { fasting: false, arrival: '08:30' }), card('retrieval-recovery', 'checklist', '회복 기록', '통증·출혈·복부팽만', { pain: 0, bleeding: 0, bloating: 0, urine: 0 }, 'emotional'), card('retrieval-count', 'result_input', '채취난자수 기록', '숫자 입력', { oocyte_count: 0 }, 'care')],
  ...
  fertilization: {
    ...
    patient: {
      ...
      checklist: [{ id: 'fert-method', label: '방법 기록' }, { id: 'fert-lab-update', label: '다음 알림' }, { id: 'fert-privacy', label: '필요한 일정만 공유' }],
      timeline: [{ id: 's4-a', label: '오늘', value: '방법 기록' }, { id: 's4-b', label: '내일', value: '알림 예정' }, { id: 's4-c', label: '공유', value: '일정만' }],
      utilityCards: [card('fert-method', 'result_input', '방법 기록', 'IVF / ICSI 선택 기록', { method: 'IVF' }, 'care'), card('fert-lab-update', 'timeline', '다음 알림', '다음 결과 알림 시간', { next_update: '내일 오전' }), card('fert-privacy', 'privacy_control', '필요한 일정만 공유', '일정만 공유', { sharing: 'basic' })],
    ...
    partner: { role: '일정 확인자', status: '일정만 공유 중', sharedContext: [{ id: 'schedule', label: '준비 시간', value: '확인' }, { id: 'privacy', label: '공유', value: '일정만' }], actions: [card('partner-sample-time', 'timeline', '준비 시간', '준비 시간', {}, 'basic'), card('partner-privacy-respect', 'support_action', '필요한 일정만 공유', '필요한 일정만 공유 중', {}, 'basic')], avoid: [{ id: 'ask-detail', label: '세부값 먼저 묻기' }], quickTools: [{ id: 'calendar', label: '일정', value: '보기' }] },
  },
  embryo_culture: {
    ...meta('embryo_culture'),
    patient: {
      ...
      checklist: [{ id: 'culture-timeline', label: '배아 업데이트' }, { id: 'culture-result', label: '결과 기록' }, { id: 'culture-share', label: '공유 범위 확인' }],
      timeline: [{ id: 'day1', label: 'Day 1', value: 'done' }, { id: 'day3', label: 'Day 3', value: 'active' }, { id: 'day5', label: 'Day 5', value: 'upcoming' }],
      utilityCards: [card('culture-timeline', 'timeline', '배아 업데이트', 'Day 1 완료 · Day 3 진행 중 · Day 5 예정', { day1: 'done', day3: 'active', day5: 'upcoming' }, 'care'), card('culture-result', 'result_input', '결과 기록', '배아 수·등급·동결 여부', { embryo_count: 0, grade: '', freeze: false }, 'emotional'), card('culture-share', 'privacy_control', '공유 범위', '결과 세부값 선택', { sharing: 'care' })],
  ...
  embryo_transfer: {
    ...
    patient: {
      ...
      checklist: [{ id: 'transfer-summary', label: '이식 요약' }, { id: 'transfer-medication', label: '약 루틴' }, { id: 'transfer-beta-date', label: 'hCG 검사일' }],
      timeline: [{ id: 's6-a', label: '오늘', value: '이식' }, { id: 's6-b', label: '매일', value: '약 루틴' }, { id: 's6-c', label: 'D+10', value: '피검' }],
      utilityCards: [card('transfer-summary', 'result_input', '이식 요약', 'Fresh/FET·배아일차·이식개수', { transfer_type: 'FET', embryo_day: 'Day5', count: 1 }, 'care'), card('transfer-medication', 'medication_card', '약 루틴', '프로게스테론 22:00 · 에스트로겐 08:00/20:00', { progesterone: false, estrogen_am: false, estrogen_pm: false }, 'care'), card('transfer-beta-date', 'timeline', 'hCG 검사일', 'hCG 검사일', { beta_date: '2026-05-29' }, 'basic')],
    ...
    partner: { role: '루틴 보조자', status: '저녁 일정 비우기', sharedContext: [{ id: 'med', label: '다음 약', value: '22:00' }, { id: 'beta', label: '피검', value: 'D+10' }], actions: [card('partner-med-assist', 'support_action', '약 시간 확인', '다음 약 시간 함께 확인', {}, 'care'), card('partner-schedule-guard', 'support_action', '저녁 일정 비우기', '저녁 일정 비우기', {}, 'basic'), card('partner-beta-countdown', 'timeline', '피검일 공유', '피검일 공유', {}, 'basic')], avoid: [{ id: 'symptom-ask', label: '증상 캐묻기' }], quickTools: [{ id: 'quiet', label: '조용한 환경', value: '준비' }] },
  },
  pregnancy_test: {
    ...meta('pregnancy_test'),
    patient: {
      ...
      checklist: [{ id: 'beta-hcg-input', label: 'hCG 결과' }, { id: 'result-visibility', label: '공유 범위' }, { id: 'next-step-planner', label: '다음 단계' }],
      timeline: [{ id: 's7-a', label: '오늘', value: '피검' }, { id: 's7-b', label: '다음', value: '2차 피검' }, { id: 's7-c', label: '이후', value: '초음파/약' }],
      utilityCards: [card('beta-hcg-input', 'result_input', 'hCG 결과', 'hCG 수치·검사 시간', { hcg: '', test_time: '09:00' }, 'care'), card('result-visibility', 'privacy_control', '공유 범위', '다음 일정만 / 결과까지 공유', { sharing: 'basic' }), card('next-step-planner', 'next_step_planner', '다음 단계', '2차 피검·초음파·약 지속', { second_beta: false, ultrasound: false, medication_continue: true }, 'basic')],
    ...
    partner: { role: '해석하지 않는 지지자', status: '결과 공유 여부 대기', sharedContext: [{ id: 'next', label: '다음 검사', value: '공유됨' }, { id: 'result', label: '결과', value: '사용자 선택' }], actions: [card('partner-result-status', 'result_input', '공유 상태', '결과 공유 여부', {}, 'care'), card('partner-next-appointment', 'timeline', '다음 검사', '다음 검사일', {}, 'basic'), card('partner-do-not-interpret', 'support_action', '하지 않기', '수치 해석하지 않기', {}, 'basic')], avoid: [{ id: 'interpret', label: '수치 해석하기' }, { id: 'predict', label: '단정하기' }], quickTools: [{ id: 'quiet', label: '말투', value: '조용히' }] },
  },
};
