# ADR 0032 — Concern triage architecture: stored signal tags, read-inline/write-route, LLM-as-classifier

## Status

Proposed — carried forward from #456 on 2026-06-10; gated by #462
...
If accepted, Concern triage would use the following boundaries. Until #462 accepts this ADR, these are design constraints for review, not implemented product behavior.
...
### 4. 캡처 진입 통합은 단계화 (proposed; no code change until accepted)

`+` 는 장기적으로 단일 진입점이 되어 사진/붙여넣기 안내문 인입까지 흡수하는 것이 목표다. 그러나 캡처 spine 은 demo-critical care loop 이므로, 케어 에이전트 신설과 캡처 진입 재배선을 동시에 하지 않는다 (ADR 0030 defer 논리 동형).

- **이번 라운드**: "주사·복약 남기기" / "병원 방문 남기기" 는 케어 에이전트 안의 **라우팅 칩**으로 유지하며 기존 `/capture` 로 보낸다. 사진/붙여넣기 입력은 `/capture` 에서 일어난다.
- **다음 라운드**: 사진/붙여넣기 affordance 를 에이전트 진입부로 끌어올린다. 단 파싱은 기존 clinic memo parsing path 를 재사용하고, 확정·저장은 split-review/confirm surface 로 라우팅한다 (쓰기 경계 유지). 채팅 안에서 파싱·확정을 끝내는 "대화형 문진" 은 금지.
...
- 사용자 카피에 "챗봇" / "감정 상담" / "걱정 정리"·"concern triage" 류 문제-명명 framing 사용. 사용자 노출 라벨은 "케어 에이전트" 로 제안하되, "에이전트" 의 자율 인상을 상쇄하기 위해 "정리만, 확정은 사용자" 비자율 계약을 UI 카피에 분명히 드러내야 한다. 진입 프롬프트는 걱정을 점화하는 "무엇이 걱정되세요?" 금지, 케어 프레임 사용.
...
- ADR 0030 — Surface Lifecycle defer/extend
- `CONTEXT.md` — Care action card and Schedule item terminology. Concern signal / Clinic question / Reminder strength remain proposed terms until #462 accepts this ADR.
