# ADR 0031 — Manual clinic logging write bridge; migrate producer before consumer

## Status

Accepted — 2026-06-07; carried forward from #456 on 2026-06-10
...
ADR 0013 did **not** migrate the **manual memo** path. Code audit on 2026-06-07 confirmed the gap that this decision was written to close:
...
At the time of decision, `care_action_cards` was fed **only** by the photo/text extraction lane. The most natural user action — open the app after a visit and type/short-note what the clinic said — wrote to `schedule_items` and never reached the canonical lane. The canonical confirmed-action table was, in practice, populated only when a user happened to take the OCR/text-analyze path. This producer-before-consumer migration was subsequently executed across #441–#447; this ADR records the decision so future work does not reintroduce a parallel `schedule_items` meaning source.
...
1. **Manual logging must produce canonical candidates by reusing the existing capture/confirm machinery.** The canonical engine already exists: `src/lib/capture-confirm-store.ts` writes `visit_inputs → action_split_drafts → split_candidates → care_action_cards` (migration `202605100003_capture_confirm.sql`), matching the CONTEXT chain Visit input → Split draft → Split candidate → Care action card. The manual memo bridge **routes through this engine**, not a parallel path. Concretely: manual memo persists a `visit_inputs` row (draft-first, raw note preserved), generates `split_candidates`, and creates `care_action_cards` only on explicit user confirm. `schedule_items` must no longer be the manual logging write target.
   ...
   - `split_candidates.visit_input_id` is `NOT NULL → visit_inputs`, so the draft must be a `visit_inputs` row. **Do not perpetuate `clinic_updates` as a parallel raw store** for the canonical path; converge manual logging onto `visit_inputs` (repo guidance: no parallel lanes for the same domain concept).
...
7. **A shared canonical care-action writer is the foundation.** #441 extracted `src/lib/canonical-care-action-writer.ts` so future structured-confirm paths do not become parallel insert paths. New code must reuse that writer rather than hand-rolling `care_action_cards` inserts.

### Migration order

0. Extract a shared canonical care-action writer from the scattered `care_action_cards` insert sites (completed in #441).
1. Build `memo → candidate/review → care_action_cards` bridge for ambiguous input; route structured manual input through the shared writer + existing confirm step (completed across #442–#443).
2. Tests proving both manual clinic-update paths (ambiguous and structured) create confirmed `care_action_cards` only after user confirmation (completed across #442–#443).
3. Migrate `/partner/[token]` read to `care_action_cards` (completed in #444).
4. Migrate `/calendar` and schedule API through a compatibility/projection layer (completed across #445–#447).
5. Keep `schedule_items` as legacy fallback / derived scheduling projection only; do not revive it as a primary meaning source.
...
- ADR 0032 — Care Agent / concern-triage front door (Proposed; do not make it a write path until accepted).
- CONTEXT.md — "Schedule item", "Care action card".
- #433 — locked product spine.
- #441–#447 — execution of this producer-before-consumer migration.
