...
| `community_posts` | #400 / #402 / ADR 0016 / ADR 0020 | records/community foundation migration + dual-scope migration + moderation pipeline | approved `audience_scope='everyone'` rows are readable by authenticated community actors; approved `same_role` rows require actor role = `audience_role`; author identity can read pending own posts and soft-delete own rows; `audience_role` is server-owned and not client-set | `records-community-schema-rls.test.ts`; `community-moderation-route-contract.test.ts`; moderation pending hidden tests |
