...
| `couple_journal_entries` | #400 / ADR 0015 / ADR 0019 | records/community foundation migration + partner-link gate migration | own couple read/update/delete through `current_user_couple_ids()`; INSERT requires Privacy Gate plus approved `partner_links.status='approved'`; partner-authored journal entries force `pain_score NULL`; photos referenced only by signed URL | `records-community-schema-rls.test.ts`; `records-journal-route-contract.test.ts`; partner pain_score NULL integration follow-up |
...
| `community_posts` | #400 / #402 / ADR 0016 / ADR 0020 | records/community foundation migration + dual-scope migration + moderation pipeline + community photo migration | approved `audience_scope='everyone'` rows are readable by authenticated community actors; approved `same_role` rows require actor role = `audience_role`; author identity can read pending own posts and soft-delete own rows; `audience_role` is server-owned and not client-set; `photo_urls` powers moderated card-style shared records, not raw clinic document sharing | `records-community-schema-rls.test.ts`; `community-moderation-route-contract.test.ts`; moderation pending hidden tests |
| `community_comments` | #400 / #402 / ADR 0016 / ADR 0020 | records/community foundation migration + dual-scope migration + moderation pipeline | comment read/write inherits visible parent post permission; cross-role comments are allowed only on `everyone` posts via parent post visibility; author identity can read pending own comments and soft-delete own rows | `records-community-schema-rls.test.ts`; `community-comments-contract.test.ts`; nested comment moderation tests |
| `community_post_empathies` | #400 / ADR 0016 / ADR 0020 | records/community foundation migration + dual-scope migration | actor couple+role only; unique `(post_id, actor_couple_id, actor_role)`; read/write follows visible parent post scope (`everyone` or matching `same_role`) | `records-community-schema-rls.test.ts`; empathy toggle/count unit tests |
...
| `community-post-photos` | #400 / ADR 0015 refresh | community photo migration | private storage bucket; uploader writes only under own `auth.uid()` folder; reads are limited to own objects or photos attached to a `can_read_community_post` visible shared record; UI/moderation prohibit raw clinic docs and visible personal information | `records-community-schema-rls.test.ts`; community photo-card contract |
...
- Records/community closed-beta surfaces must preserve actor-specific boundaries: couple journal is couple-only and write-gated by an approved partner link; community feed reads are `approved` plus ADR 0020 scope (`everyone` or role-matched `same_role`), and pending author rows are visible only to their own identity.
...
- Records/community schema contract covers table creation, RLS enablement, dual-scope community reads, partner-link-gated journal insert, partner `pain_score NULL`, storage bucket privacy, and service-role audit contract.
