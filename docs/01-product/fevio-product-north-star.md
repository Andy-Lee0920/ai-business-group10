...
### Validated customer experience jobs

Fevio가 풀어야 하는 고객경험은 다음 10가지로 고정한다. 이 목록은 기능 우선순위와 화면 리뷰의 기준이다.

1. **오늘 해야 할 일을 바로 보는 것** — 병원 안내문, 카톡, 사진, 메모를 다시 뒤지지 않고 오늘 주사·약·내원·확인할 것만 본다.
2. **직접 입력하는 귀찮음을 줄이는 것** — 약 이름, 시간, 용량, 병원 방문 일정을 캘린더에 일일이 옮기는 부담을 줄인다.
3. **자동 정리는 원하지만 최종 확인권은 본인이 갖는 것** — AI/OCR은 후보를 만들고, 사용자는 원문과 비교한 뒤 확인·저장한다.
4. **중요한 정보를 놓치지 않는 것** — 주사 시간, 약 이름, 용량처럼 시간 민감한 정보는 조용하지만 강하게, 정확하게 알려준다.
5. **파트너가 무엇을 도와야 하는지만 알게 하는 것** — 전체 의료정보가 아니라 동행, 주사 준비, 확인 같은 역할만 공유한다.
6. **민감한 정보는 스스로 통제하는 것** — 검사 결과, 감정 기록, 병원 원문, 상세 의료기록은 자동 공유하지 않는다.
7. **처음 시험관을 시작할 때 복잡한 과정을 쉽게 이해하는 것** — 채취, 배양, 이식 같은 용어와 현재 단계를 짧게 이해한다.
8. **앱이 재촉하거나 감정적으로 부담 주지 않는 것** — 알림은 정확하되 과하지 않고, 감정 케어를 강요하지 않는다.
9. **시술 기간 동안만 집중적으로 쓰는 실용적인 도구가 되는 것** — 평소 매일 쓰는 앱이 아니라, 일정과 주사가 많은 기간에 확실히 필요해야 한다.
10. **유료 가치가 단순 캘린더를 넘어서는 것** — 사진/문자 자동 정리, 원문 확인, 중요 알림, 파트너 역할 공유가 편의와 신뢰를 만든다.
