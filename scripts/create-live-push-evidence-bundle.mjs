...
const platforms = args.platform === 'both' ? ['android', 'ios'] : [args.platform];
for (const platform of platforms) {
  const folderName = `${platform}-live-push-evidence`;
  const bundleDir = join(args.outDir, folderName);
  mkdirSync(bundleDir, { recursive: true });
  writeFileSync(join(bundleDir, 'README.md'), renderReadme(platform));
...
if (args.platform === 'both') {
  ...
  writeFileSync(join(args.outDir, 'README.md'), renderCombinedReadme());
}
...
function renderReadme(platform) {
  ...
  const finalCompletionCommand = renderFinalCompletionCommand();
...
function renderCombinedReadme() {
  const finalCompletionCommand = renderFinalCompletionCommand();
  return `# Fevio Android/iOS live push evidence bundle\n\nCollect both platform folders before closing #382/#383 or completing the MVP goal. Each platform folder has its own per-device closure guard. After both pass, run the final gate from this directory.\n\n\`\`\`bash\n${finalCompletionCommand}\n\`\`\`\n`;
}

function renderFinalCompletionCommand() {
  return [
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--platform' && next && ['android', 'ios', 'both'].includes(next)) { parsed.platform = next; index += 1; continue; }
