...
describe('SLC home screen vertical slices', () => {
  ...
  it('uses a medication-first full-bleed care-state hero instead of an embryo story surface', () => {
    ...
    expect(markup).toContain('data-testid="home-sheet-handle"');
    expect(markup).not.toContain('확인할 항목은 아래에 접어뒀어요');
    expect(markup).not.toContain('오늘 필요한 실행만 먼저 보고');
    expect(markup).not.toContain('data-testid="home-care-ops-promise"');
    expect(markup).not.toContain('오늘 실행 기준');
    expect(markup).not.toContain('원문 확인 후 저장');
    expect(markup).not.toContain('파트너 역할만');
