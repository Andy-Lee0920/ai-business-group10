...
describe('PartnerRoleSurface first fold', () => {
  it('leads with one actionable “오늘 도와줄 일” card and allows non-injection assist CTA', () => {
    ...
    const markup = renderToStaticMarkup(
      <PartnerRoleSurface
        items={[
          item({ safe_id: 'completed-injection', card_type: 'injection', display_state: 'completed', partner_action: '이미 도왔어요.' }),
          item({ safe_id: 'medication-help', card_type: 'medication', partner_action: '물과 알람을 같이 확인해 주세요.' }),
        ]}
        onAssist={onAssist}
      />,
    );
