...
describe('sensitive care write seam', () => {
  it('authenticates, checks Privacy Gate, creates visit input, then creates one confirmed care action card', async () => {
    const visitTable = createInsertTable({ id: 'visit-1' });
    const cardTable = createCareActionInsertTable({
      id: 'card-1',
      status: 'confirmed',
    });
    const from = vi.fn((table: string) =>
      table === 'visit_inputs' ? visitTable : cardTable,
    );
    const supabase = {
      auth: {
      ...
      rpc: vi.fn().mockResolvedValue({
        data: [
          {
            couple_id: 'couple-1',
            privacy_gate_accepted_at: '2026-05-11T00:00:00.000Z',
          },
        ],
        error: null,
      }),
    ...
    expect(visitTable.insert).toHaveBeenCalledWith({
      couple_id: 'couple-1',
      raw_text: '민감 케어 입력',
    });
    expect(cardTable.insert).toHaveBeenCalledWith(
  ...
  it('rejects before any sensitive table write when Privacy Gate is missing', async () => {
    ...
    const supabase = {
      ...
      rpc: vi.fn().mockResolvedValue({
        data: [{ couple_id: 'couple-1', privacy_gate_accepted_at: null }],
        error: null,
      }),
      from,
    } as unknown as SensitiveCareWriteSupabaseClient;
    ...
    await expect(
      createSensitiveCareActionCard(supabase, {
        sourceText: '민감 케어 입력',
        card: {
          assignee_role: 'primary_user',
          card_type: 'record',
