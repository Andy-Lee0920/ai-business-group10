...
import {
  createConfirmedCareAction,
  type CanonicalCareActionCreate,
  type CanonicalCareActionRow,
  type CanonicalCareActionWriterClient,
} from './canonical-care-action-writer';
import {
  bootstrapCoupleForUserWithServiceRole,
  isInitCoupleAmbiguityError,
} from './couple-bootstrap-admin';
...
export type SensitiveCareWriteSupabaseClient = {
  ...
  from(
    ...
    CanonicalCareActionWriterClient<CanonicalCareActionRow>['from']
  >;
};
...
export type SensitiveCareCardDraft = Omit<
  CanonicalCareActionCreate,
  | 'couple_id'
  | 'created_by'
  | 'source_input_id'
  | 'source_text'
...
export async function createSensitiveCareActionCard(
  ...
  assertSensitiveWriteAllowed({
    privacyGateAcceptedAt: bootstrap.privacy_gate_accepted_at,
  });

  const visitInput = await supabase
    .from('visit_inputs')
    .insert<VisitInputRow>({
      couple_id: bootstrap.couple_id,
      raw_text: input.sourceText,
    })
    .select('id')
    .single();
  if (visitInput.error || !visitInput.data)
    throw new Error(visitInput.error?.message ?? 'visit_inputs insert failed');
...
export function isPrivacyGateRequiredError(error: unknown): error is Error {
  return (
    ...
    error.message.includes('Privacy Gate must be accepted')
  );
}

export function isMissingSupabasePublicConfigError(
  error: unknown,
): error is Error {
  return (
    error instanceof Error &&
    error.message.includes('Missing Supabase public config')
  );
}

async function getAuthenticatedUser(
  supabase: SensitiveCareWriteSupabaseClient,
): Promise<SensitiveCareUser> {
  const userResult = await supabase.auth.getUser();
  if (userResult.error || !userResult.data.user)
    throw new Error(userResult.error?.message ?? 'Authentication required.');
  return userResult.data.user;
}

async function bootstrapSensitiveContext(
  supabase: SensitiveCareWriteSupabaseClient,
  user: SensitiveCareUser,
