  ...
  return (
    <div style={{ minHeight: '100dvh', padding: '60px 0 112px', background: 'var(--slc-bg)' }}>
      <div style={{ padding: '0 24px 16px' }}>
        <h1 style={{ fontSize: 28, fontWeight: 900, color: 'var(--slc-text)', margin: '0 0 18px', letterSpacing: '-0.03em' }}>기록</h1>
      ...
      {groups.length === 0 ? (
        <div style={{ padding: '56px 24px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--slc-muted)', fontSize: 15, lineHeight: 1.55, margin: 0 }}>{SLC_SAFE_COPY.emptyRecords}</p>
        </div>
      ) : (
...
function TimelineRecord({ record, isLast }: { record: RecordsViewRecord; isLast: boolean }) {
  ...
  return (
    <article style={{ position: 'relative', display: 'grid', gridTemplateColumns: '34px 1fr', gap: 12 }}>
      ...
      <div style={{ minHeight: 86, background: '#FFFBF8', border: '1px solid var(--slc-border)', borderRadius: 18, padding: '14px 16px', boxShadow: '0 6px 20px rgba(80, 50, 40, 0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
          <div>
            <p style={{ fontSize: 13, color: '#8A7B73', fontWeight: 800, margin: '0 0 6px' }}>{formatTime(record.at)}</p>
            <p style={{ fontSize: 16, color: 'var(--slc-text)', fontWeight: 900, lineHeight: 1.25, margin: '0 0 5px' }}>{record.title}</p>
            <p style={{ fontSize: 12, color: 'var(--slc-muted)', lineHeight: 1.45, margin: 0 }}>{record.meta}</p>
          </div>
...
function filterChipStyle(active: boolean) {
  return {
    ...
    background: active ? 'var(--slc-coral)' : 'var(--slc-border)',
